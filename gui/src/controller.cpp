...
void Controller::saveScroll(const QString &jid, int index)
{
    if (!jid.isEmpty()) {
        m_scroll.insert(jid, index);
    }
}

int Controller::savedScroll(const QString &jid) const
{
    return m_scroll.value(jid, -1);
}
