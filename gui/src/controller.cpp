...
void Controller::requestContactInfo(const QString &jid)
{
    m_ipc->requestContactInfo(jid);
}

void Controller::onContactInfoReceived(const QJsonObject &info)
{
    m_contactInfo = info.toVariantMap();
    Q_EMIT contactInfoChanged();
}
